 ...
 * @author kevin
 */
public class ServletSortiePatientAccueil_Dose2 extends HttpServlet {
     ...
     if(session.getAttribute("role").toString().equals("accueil")){
        ...
        v.setNum_Lot(vacAtt);  
            try {
                ...
                Logger.getLogger(ServletSortiePatientAccueil_Dose2.class.getName()).log(Level.SEVERE, null, ex);
            }
        
        citoyen.vuePatient(c);
        if(citoyen.getVaccin_attribue().equals(vac.getNom())){
            citoyen.vaccination_D2(c);
